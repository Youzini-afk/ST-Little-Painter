...
The tagger prompt composer uses a BME-inspired ordered-message layout instead of flattening all context into one large JSON user message. It sends BME-style identity header blocks, prompt profile guidance, worldbook before/after as real `system` messages, character context, role-mixed chat history, at-depth worldbook messages using their resolved roles, selected skills, and dictionary hints. The latest assistant reply is also repeated in an explicit `Latest assistant reply - anchor source` block so `anchorQuote` is easy to copy from the current target reply. Prompt composer truncation is limited to the configurable recent chat message count; worldbook, scene plan, character fields, and at-depth messages are not character-truncated by the composer.
