...
function createUnavailableFacade(name) {
  return Object.freeze({
    available: false,
    mode: "unavailable",
    fallbackReason: `${name} host facade is not vendored for Little Painter worldbook resolution`,
    versionHints: Object.freeze({ scope: "st-little-painter-worldbook-only" }),
  });
}
...
export function createHostAdapter(options = {}) {
  ...
  const regex = createUnavailableFacade("regex");
  const injection = createUnavailableFacade("injection");
  const adapter = {
    context,
    worldbook,
    regex,
